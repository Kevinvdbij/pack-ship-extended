...
<template>
	<div v-if="renderComment()">
		<Transition>
			<div v-show="show" style="position: relative; right: 30px; margin-top: 32px;">
				<Transition>
					<div class="mt-2 alert alert-info comment-alert"
						style="text-align: center; font-size: 24px; max-width:100%;color: #7f5353;background-color: #f6efef;border-color: #7f5353;"
						v-show="hasComment()"><b>Let op!</b> Er is een shopware notitie.</div>
				</Transition>
				<div class="card sw-card" v-show="show">
					...
					<div class="sw-body">
						<div class="card-body sw-body sw-content">
							<div class="loader"
								style="position: relative; margin-top: -30px; top: 45px; line-height: 20px; justify-self: center;"
								v-show="!swCommentBoxEnabled"></div>
							<textarea class="text-box sw-textarea" :disabled="!swCommentBoxEnabled"
