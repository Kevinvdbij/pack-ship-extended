...
//
// The reservations that are not fully picked are included, and they may be the
// ones this matters most for: a half-picked order is exactly what has to stand
// somewhere until the rest turns up. They cost an extra step to identify --
// see `identify()` -- because the portal will not tell us anything about them at
// all, not even an id.
//
...
// discuss it, so those are looked up by number through the ERP's own reservation
// search: `findReservationId()`, which costs a few page loads in the shared
// frame and is remembered afterwards.
//
// A card we cannot place keeps its panel and says it has nothing, rather than
